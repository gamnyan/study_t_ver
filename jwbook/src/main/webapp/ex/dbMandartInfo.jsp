...
<body>
	<h2>점심메뉴</h2>
	<%-- <td>${dbMandarts.id }</td> --%>
	<c:forEach var="dm" items="${dbMandarts}">
		<c:if test="${dm.id==2}">
			<td><input type="submit" name="goal" value="${dm.chinese}"></td>
		</c:if>
	</c:forEach>

	<form method="get">
		<table border="1">
			<c:forEach var="dm" items="${dbMandarts}" varStatus="i" begin="0"
				end="2" step="1">
				<tr>
					<c:forEach varStatus="j" begin="0" end="2" step="1">
						<c:if test="${dm.id==(3 * i.index + j.index)}">
							<td><input type="submit" name="goal" value="${dm.chinese}"></td>
						</c:if>
						<%-- <c:choose>
	...
	<%-- <form method="get">
		<table border="1">
			<c:forEach items="dbMandarts" var="dm" varStatus="i" begin="0" end="2" step="1">
				<tr>
					<c:forEach varStatus="j" begin="0" end="2" step="1">
						<c:choose>
							<c:when test="${3*i.index+j.index==4}">
								<td>${dm.getKey(3 * i.index + j.index) }</td>
							</c:when>
							<c:otherwise>
								<td><input type="submit" name="goal"
									value="${dm.getKey(3 * i.index + j.index)}"></td>
							</c:otherwise>
						</c:choose>
					</c:forEach>
				</tr>
	...
	<!-- <form method="get" action="/jwbook/mandartControl?action=list"> -->
	<%-- <table border="1">
		<c:forEach items="${dbMandarts}" var="dm">
				...
				<tr>
					<c:forEach varStatus="j" begin="0" end="2" step="1">
						<c:choose>
							...
							<c:otherwise>
								<td><input type="submit" name="goal"
									value="${dm.chinese(3 * i.index + j.index)}"></td>
							</c:otherwise>
						</c:choose>
					</c:forEach>
				</tr>
